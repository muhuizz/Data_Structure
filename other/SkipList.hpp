...
public:
		...
		while (lev >= 0)
		{
			while (cur->_vec[lev])
			{
				...
				else		// cur->_vec[lev]->_key == key，准备删除该Node
				{
					...
					// 调整最大level
					if (dst->_level == _maxLevel)	// 只有当删除结点的level最大时，才需要调整
					{
						int tmp = MAXLEVEL - 1;
						cur = _head;
						while (tmp >= 0 && cur->_vec[tmp] == NULL)
							tmp--;						
						_maxLevel = tmp + 1;;
					}
	...
	{
		if (_head->_vec[0] == NULL)
			return false;

		int index = _maxLevel-1;
		Node* cur = _head;
		while (index >= 0)
		{
			while (cur->_vec[index] != NULL)
			{
				if (key > cur->_vec[index]->_key)
					cur = cur->_vec[index];
				else if (key < cur->_vec[index]->_key)
					break;
				else
					return true;
			}
